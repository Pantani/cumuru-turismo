...
if (document.info.version !== EXPECTED_CONTRACT_VERSION) {
  throw new Error(
    `analytics contract version drifted: ${document.info.version}, esperado ${EXPECTED_CONTRACT_VERSION}`,
  );
}
requireRef(
...
cleanup() {
  ...
  if test "${inspect_status}" -ne 0 || test -n "${residual}"; then
    echo "analytics proxy left containers behind" >&2
    cleanup_status=1
  fi
  residual="$(
  ...
  inspect_status=$?
  if test "${inspect_status}" -ne 0 || test -n "${residual}"; then
    echo "analytics proxy left its network behind" >&2
    cleanup_status=1
  fi
  residual="$(
  ...
  inspect_status=$?
  if test "${inspect_status}" -ne 0 || test -n "${residual}"; then
    echo "analytics proxy left its volume behind" >&2
    cleanup_status=1
  fi
  ...
  echo "analytics proxy cleanup confirmed"
}
trap cleanup EXIT
...
require_running() {
  ...
  if ! "${COMPOSE[@]}" ps --status running --services |
    ...
    echo "analytics proxy service is not running: ${service}" >&2
    return 1
  fi
}
...
request() {
  ...
  LAST_STATUS="$(
    "${COMPOSE[@]}" exec -T \
      ...
      web sh -eu -c '
        : > /tmp/analytics-response.body
        set -- \
          --silent \
          --show-error \
          --dump-header /tmp/analytics-response.headers \
          --output /tmp/analytics-response.body \
          --write-out "%{http_code}"
  ...
  LAST_HEADERS="$(
    "${COMPOSE[@]}" exec -T web \
      sh -eu -c 'cat /tmp/analytics-response.headers'
  )"
  LAST_BODY="$(
    "${COMPOSE[@]}" exec -T web \
      sh -eu -c 'cat /tmp/analytics-response.body'
  )"
}
...
if test "${ready}" != "true"; then
  echo "timed out waiting for analytics public analytics" >&2
  exit 1
fi
...
CAPABILITY_CANARY="analytics-capability-canary-4ca7f2"
QUERY_CANARY="analytics-query-canary-19b83e"
request \
...
assert_absent \
  "${RUNTIME_LOG}" \
  "${CAPABILITY_CANARY}" \
  "analytics runtime logs leaked the synthetic capability"
assert_absent \
  "${RUNTIME_LOG}" \
  "${QUERY_CANARY}" \
  "analytics runtime logs leaked the synthetic query"
assert_absent \
  "${RUNTIME_LOG}" \
  "private-runtime-canary-f4-72f51c" \
  "analytics runtime logs leaked private database data"

echo "ANALYTICS_PROXY_RUNTIME=PASS"
