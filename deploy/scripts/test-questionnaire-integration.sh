...
COMPOSE=(
  ...
  --file "${ROOT_DIR}/deploy/compose.questionnaire-integration.yaml"
...
cleanup() {
  ...
  if test "${inspect_status}" -ne 0 || test -n "${residual_containers}"; then
    echo "questionnaire integration cleanup left containers or could not inspect" >&2
    cleanup_status=1
  fi
  residual_networks="$(
  ...
  if test "${inspect_status}" -ne 0 || test -n "${residual_networks}"; then
    echo "questionnaire integration cleanup left its network or could not inspect" >&2
    cleanup_status=1
  fi
  residual_volumes="$(
  ...
  if test "${inspect_status}" -ne 0 || test -n "${residual_volumes}"; then
    echo "questionnaire integration cleanup left its volume or could not inspect" >&2
    cleanup_status=1
  fi
  set -e
...
SET ROLE :"probe_role";
DO $questionnaire_denied_operations$
DECLARE
  operation text;
  statement text;
  target_table text;
BEGIN
  ...
  END LOOP;
END
$questionnaire_denied_operations$;
SQL
}
...
psql_as postgres cumuru-local-admin-only \
  --command='CREATE ROLE questionnaire_public_probe NOLOGIN'
assert_denied_matrix \
  postgres \
  cumuru-local-admin-only \
  questionnaire_public_probe
psql_as postgres cumuru-local-admin-only \
  --command='DROP ROLE questionnaire_public_probe'
...
if ! postgres_sink_logs="$(
  ...
  echo "could not inspect the questionnaire PostgreSQL log sink" >&2
  exit 1
fi
for canary in \
  "${CAPABILITY_CANARY}" \
  ...
  if grep --fixed-strings --quiet "${canary}" <<<"${postgres_sink_logs}"; then
    echo "questionnaire canary reached the PostgreSQL log sink" >&2
    exit 1
  fi
done
...
echo "questionnaire PostgreSQL integration passed with consolidated migration baseline"
