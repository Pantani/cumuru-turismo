...
# Fase 7 — autoatendimento e aprovação, stack completa.
#
...
fail() {
  echo "PHASE7_FULL_STACK=FAIL $1" >&2
  exit 1
}

# O contrato é a fonte: as operações exercitadas aqui são exatamente as que o
# OpenAPI declara como fase 7, então acrescentar operação sem teste quebra esta
# asserção antes de quebrar o gate.
declared_operations="$(
  grep -c 'x-cumuru-implementation-phase: 7' "${ROOT_DIR}/contracts/openapi.yaml" || true
)"
if test "${declared_operations}" -ne 10; then
  fail "the contract declares ${declared_operations} phase 7 operations, expected 10"
fi
...
# que é quem alcança o serviço, no mesmo padrão do gate da Fase 4.
#
...
#
# Sem PHASE7_ENABLED as rotas do canal aberto não são registradas, e TODA
...
phase_probe="$(request_status \
  "the phase 7 preflight" OPTIONS /api/v1/accommodation-invite \
  "Origin: ${ORIGIN}" "Access-Control-Request-Method: GET")"
if test "${phase_probe}" = "404"; then
  fail "PHASE7 IS DISABLED in this stack: the open channel routes are not registered, so every assertion below would pass by absence of route. Set PHASE7_ENABLED and the proof-of-work keyring in the Compose files."
fi
expect_status "the phase 7 preflight" "204" "${phase_probe}"
...
preflight_headers="$(request_headers \
  "the phase 7 preflight allowlist" OPTIONS /api/v1/accommodation-invite \
  "Origin: ${ORIGIN}" "Access-Control-Request-Method: GET")"
...
# da Fase 2 não usa cookie de propósito — é o que faz Idempotency-Key e JSON
...
issue_poster() {
  ...
  web_request POST "/api/v1/accommodations/${target}/invite" \
    ...
    "${authorization}" \
    "If-Match: ${etag}" \
    "Idempotency-Key: phase7-full-stack-poster-${suffix}"
}
...
solution="$(node "${ROOT_DIR}/deploy/scripts/phase7-solve-pow.mjs" \
...
submit_status() {
  ...
  web_request POST /api/v1/accommodation-invite/submit "$(printf '%s' "${body}" | tr -d '\n')" \
    ...
    "Idempotency-Key: phase7-full-stack-submit-${key}"
}
...
foreign_solution="$(node "${ROOT_DIR}/deploy/scripts/phase7-solve-pow.mjs" \
