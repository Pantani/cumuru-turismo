...
PROJECT_NAME="cumuru-phase4-integration-${PPID}-$$"
. "${ROOT_DIR}/deploy/scripts/lib/compose-subnet.sh"
cumuru_acquire_subnet phase4-integration
COMPOSE=(
  "${ROOT_DIR}/deploy/scripts/with-build-metadata.sh"
  docker compose
  --file "${ROOT_DIR}/compose.yaml"
  --file "${ROOT_DIR}/deploy/compose.phase4-integration.yaml"
...
cleanup() {
  ...
  if test "${inspect_status}" -ne 0 || test -n "${residual}"; then
    echo "phase 4 integration left containers behind" >&2
    cleanup_status=1
  fi
  residual="$(
  ...
  inspect_status=$?
  if test "${inspect_status}" -ne 0 || test -n "${residual}"; then
    echo "phase 4 integration left its network behind" >&2
    cleanup_status=1
  fi
  residual="$(
  ...
  inspect_status=$?
  if test "${inspect_status}" -ne 0 || test -n "${residual}"; then
    echo "phase 4 integration left its volume behind" >&2
    cleanup_status=1
  fi
  set -e
  ...
  echo "phase 4 integration cleanup confirmed"
}
trap cleanup EXIT
...
VALUES (
  ...
  'phase4_preference_window',
  'Phase 4 preference window fixture'
);
...
expect_failure \
  ...
  "CREATE TEMPORARY TABLE phase4_public_temp_canary (value integer)"

expect_failure \
  cumuru_public \
  cumuru-local-public-only \
  "public login created a schema" \
  "CREATE SCHEMA phase4_public_schema_canary"
...
CREATE ROLE phase4_public_elevated
  NOLOGIN CREATEDB CREATEROLE BYPASSRLS;
GRANT phase4_public_elevated TO cumuru_public;
SQL
...
elevated_role_risk="$(
  scalar_as \
    ...
    cumuru-local-public-only \
    "RESET ROLE;
     SET ROLE phase4_public_elevated;
     SELECT
       current_user,
       rolcreatedb,
       rolcreaterole,
...
test "${elevated_role_risk}" = \
  "phase4_public_elevated|t|t|t"

psql_as postgres cumuru-local-admin-only <<'SQL'
REVOKE phase4_public_elevated FROM cumuru_public;
DROP ROLE phase4_public_elevated;
SQL
...
echo "PHASE4_POSTGRES_INTEGRATION=PASS"
