...
PROJECT_NAME="cumuru-phase2-integration-${PPID}-$$"
. "${ROOT_DIR}/deploy/scripts/lib/compose-subnet.sh"
cumuru_acquire_subnet phase2-integration
COMPOSE=(
  "${ROOT_DIR}/deploy/scripts/with-build-metadata.sh"
  docker compose
  --file "${ROOT_DIR}/compose.yaml"
  --file "${ROOT_DIR}/deploy/compose.phase2-integration.yaml"
...
cleanup() {
  ...
  if test "${inspect_status}" -ne 0; then
    echo "phase 2 integration cleanup could not inspect containers" >&2
    cleanup_status=1
  elif test -n "${residual_containers}"; then
    echo "phase 2 integration cleanup left containers behind" >&2
    cleanup_status=1
  fi
  residual_networks="$(
  ...
  inspect_status=$?
  if test "${inspect_status}" -ne 0; then
    echo "phase 2 integration cleanup could not inspect networks" >&2
    cleanup_status=1
  elif test -n "${residual_networks}"; then
    echo "phase 2 integration cleanup left its network behind" >&2
    cleanup_status=1
  fi
  residual_volumes="$(
  ...
  inspect_status=$?
  if test "${inspect_status}" -ne 0; then
    echo "phase 2 integration cleanup could not inspect volumes" >&2
    cleanup_status=1
  elif test -n "${residual_volumes}"; then
    echo "phase 2 integration cleanup left its volume behind" >&2
    cleanup_status=1
  fi
  set -e
  ...
  echo "phase 2 integration cleanup confirmed"
}
trap cleanup EXIT
...
echo "phase 2 PostgreSQL integration passed with cumuru_app runtime role"
