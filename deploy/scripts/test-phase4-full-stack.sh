...
AFTER_HEADERS="$(mktemp "${TMPDIR:-/tmp}/cumuru-phase4-after-headers.XXXXXX")"
AFTER_BODY="$(mktemp "${TMPDIR:-/tmp}/cumuru-phase4-after-body.XXXXXX")"
METHODOLOGY_BODY="$(mktemp "${TMPDIR:-/tmp}/cumuru-phase4-methodology.XXXXXX")"
INVALID_DSN_LOG="$(mktemp "${TMPDIR:-/tmp}/cumuru-phase4-invalid-dsn.XXXXXX")"
WORKER_LOG="$(mktemp "${TMPDIR:-/tmp}/cumuru-phase4-worker.XXXXXX")"
...
cleanup() {
  ...
  if test "${inspect_status}" -ne 0 || test -n "${residual}"; then
    echo "phase 4 full-stack left containers behind" >&2
    cleanup_status=1
  fi
  residual="$(
  ...
  inspect_status=$?
  if test "${inspect_status}" -ne 0 || test -n "${residual}"; then
    echo "phase 4 full-stack left its network behind" >&2
    cleanup_status=1
  fi
  residual="$(
  ...
  inspect_status=$?
  if test "${inspect_status}" -ne 0 || test -n "${residual}"; then
    echo "phase 4 full-stack left its volume behind" >&2
    cleanup_status=1
  fi
  rm -f -- \
    "${CONFIG_FILE}" \
  ...
  echo "phase 4 full-stack cleanup confirmed"
}
trap cleanup EXIT
...
require_running() {
  ...
  if ! "${COMPOSE[@]}" ps --status running --services |
    ...
    echo "phase 4 full-stack service is not running: ${service}" >&2
    return 1
  fi
}
...
request_public() {
    ...
    "${COMPOSE[@]}" exec -T \
      ...
      web sh -eu -c '
        curl \
          --silent \
          --show-error \
          --dump-header /tmp/phase4-full-stack.headers \
          --output /tmp/phase4-full-stack.body \
          --write-out "%{http_code}" \
  ...
  test "${status}" = "200"
  "${COMPOSE[@]}" exec -T web \
    sh -eu -c 'cat /tmp/phase4-full-stack.headers' >"${headers_path}"
  "${COMPOSE[@]}" exec -T web \
    sh -eu -c 'cat /tmp/phase4-full-stack.body' >"${body_path}"
}
...
if (api.environment.PHASE4_ENABLED !== "true") {
  throw new Error("phase 4 is disabled for api");
}
if (worker.environment.PHASE4_ENABLED !== "true") {
  throw new Error("phase 4 is disabled for worker");
}
if (worker.environment.PHASE4_INCREMENTAL_INTERVAL !== "500ms") {
  throw new Error("phase 4 worker cadence is not test-bounded");
}
...
if test "${publication_ready}" != "true"; then
  echo "worker did not create the initial phase 4 publication" >&2
  exit 1
fi
...
echo "PHASE4_LAST_VALID_SNAPSHOT=PASS"
echo "PHASE4_FULL_STACK_RUNTIME=PASS"
