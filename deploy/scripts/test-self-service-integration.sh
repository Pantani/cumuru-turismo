...
COMPOSE=(
  ...
  --file "${ROOT_DIR}/deploy/compose.self-service-integration.yaml"
...
acquire_gate_lock() {
  ...
  while ! mkdir "${GATE_LOCK_DIR}" 2>/dev/null; do
    ...
    if test "${SECONDS}" -ge "${deadline}"; then
      echo "self-service integration timed out waiting for Docker subnet lock" >&2
      return 1
    fi
    sleep 1
  done
...
INSERT INTO core.organizations (id, name, legal_name) VALUES (
  ...
  'Organização Fictícia autoatendimento',
  'Organização Fictícia autoatendimento LTDA'
);
...
VALUES (
  ...
  '00000000-0000-7000-8000-0000000007a1',
  'Pousada Fictícia autoatendimento',
  'active',
  'formal_lodging',
  10
);
...
assert_equals "core.stays lost a Self-service approval column" "3" "${approval_projection}"
...
if psql_as cumuru_public cumuru-local-public-only \
  ...
  echo "public_runtime unexpectedly read core.stays after the Self-service migration" >&2
  exit 1
fi
...
-- A publicação respeita a máquina de estados do questionário:
...
# alcançasse a função — que é o modo de falha que este teste encontrou oito vezes.
...
echo "self-service PostgreSQL integration passed: unlimited-invite silent failure reproduced and fixed, invite targeting, stay provenance and approval invariants, pending account credential state, proof-of-work nonce ledger and the approval filter in the SECURITY DEFINER aggregate proved by calling it: pending excluded, approved counted"
