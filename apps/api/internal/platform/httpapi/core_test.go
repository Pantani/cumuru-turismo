...
func TestAccommodationOnboardingReturnsContractHeaders(t *testing.T) {
	...
	handler := coreHandlerWithOptions(t, coreHandlerOptions{
...
func assertAccommodationOnboardingHTTPCase(
	...
	t.Helper()
	handler := coreHandlerWithOptions(t, coreHandlerOptions{onboardingEnabled: tt.enabled})
...
func TestStayMutationResponsesAreMinimal(t *testing.T) {
	...
	handler := coreHandlerWithStay(t, stayRepositoryStub{
...
func TestIdempotencyProcessingHasRetryAfterButHashConflictDoesNot(t *testing.T) {
	...
	processing := coreHandlerWithStay(t, stayRepositoryStub{
		createErr: idempotency.NewProcessingError(3 * time.Second),
	})
	assertCreateStayConflict(t, processing, "3")

	hashConflict := coreHandlerWithStay(t, stayRepositoryStub{
...
func TestConsumedInviteUsesSpecificConflictProblem(t *testing.T) {
	...
	handler := coreHandlerWithStay(t, stayRepositoryStub{submitErr: stay.ErrInviteConsumed})
...
func coreHandler(t *testing.T) http.Handler {
	t.Helper()
	return coreHandlerWithStay(t, stayRepositoryStub{})
}

func coreHandlerWithStay(t *testing.T, stays stay.Repository) http.Handler {
	t.Helper()
	return coreHandlerWithOptions(t, coreHandlerOptions{stays: stays})
}
...
func coreHandlerWithOptions(t *testing.T, options coreHandlerOptions) http.Handler {
	t.Helper()
