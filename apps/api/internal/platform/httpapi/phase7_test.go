...
const (
	...
	phase7StayID    = "019f0000-0000-7000-8000-000000000001"
	phase7AccommoID = "019f0000-0000-7000-8000-000000000002"
)

type phase7StayStub struct {
	stay.Repository
...
func (s phase7StayStub) SubmitSelfRegistration(
	_ context.Context,
	...
	return stay.SelfRegistrationAccepted{
		SubmissionID: uuid.MustParse(phase7StayID), StayID: uuid.MustParse(phase7StayID),
		Status: "accepted", StayStatus: stay.StatusPreRegistered,
		ApprovalState: stay.ApprovalPending, Version: 1,
	}, false, nil
}

func (s phase7StayStub) GetAccommodationInviteContext(
	_ context.Context,
...
type phase7ActivationStub struct {
	command *activation.CompleteCommand
}

func (phase7ActivationStub) Create(
	_ context.Context,
	command activation.CreateCommand,
) (activation.Created, bool, error) {
	return activation.Created{
		ActivationID: uuid.MustParse(phase7StayID), AccountID: uuid.MustParse(phase7AccommoID),
		URL: "http://127.0.0.1:4173/ativacao#" + posterToken, Version: 1,
	}, false, nil
}

func (phase7ActivationStub) Context(
	context.Context,
	activation.Request,
...
func (s phase7ActivationStub) Complete(
	_ context.Context,
	command activation.CompleteCommand,
) error {
	if s.command != nil {
...
func phase7Request(method, target, token, body string) *http.Request {
...
// This is the regression the phase exists to prevent, and it must fail at the
// transport, before any repository is reached.
func TestApprovalRefusesTheEditScope(t *testing.T) {
	t.Parallel()

	handler := phase7Handler(t, phase7StayStub{}, nil)
	for _, route := range []string{"approve", "reject"} {
		request := phase7Request(
			http.MethodPost, "/api/v1/stays/"+phase7StayID+"/"+route, "writer", "{}",
		)
...
func TestApprovalAcceptsItsOwnScopeAndCarriesTheVersion(t *testing.T) {
	...
	handler := phase7Handler(t, phase7StayStub{approvalCommand: &captured}, nil)
	request := phase7Request(
		http.MethodPost, "/api/v1/stays/"+phase7StayID+"/approve", "approver", "{}",
	)
	request.Header.Set("If-Match", `"3"`)
	request.Header.Set("Idempotency-Key", "approval-key-0000000001")
	recorder := httptest.NewRecorder()
	...
	if captured.ExpectedVersion != 3 || captured.StayID.String() != phase7StayID {
...
func TestRejectionRefusesFreeText(t *testing.T) {
	...
	for body, want := range bodies {
		request := phase7Request(
			http.MethodPost, "/api/v1/stays/"+phase7StayID+"/reject", "approver", body,
		)
		request.Header.Set("If-Match", `"3"`)
...
func TestListStaysCarriesTheApprovalFilters(t *testing.T) {
	...
	handler := phase7Handler(t, phase7StayStub{page: &captured}, nil)
	target := "/api/v1/stays?accommodation_id=" + phase7AccommoID +
		"&approval_state=pending&provenance=self_service"
	recorder := httptest.NewRecorder()
	handler.ServeHTTP(recorder, phase7Request(http.MethodGet, target, "approver", ""))
...
func TestListStaysRefusesAnUnknownApprovalFilter(t *testing.T) {
	...
	for _, query := range []string{
		...
		recorder := httptest.NewRecorder()
		handler.ServeHTTP(recorder, phase7Request(
...
func TestPosterContextReadsTheCapabilityHeader(t *testing.T) {
	...
	handler := phase7Handler(t, phase7StayStub{posterRequest: &captured}, nil)
...
func TestSelfRegistrationRefusesIdentityFields(t *testing.T) {
	...
	for _, field := range []string{
		...
		body := `{"client_submission_id":"` + phase7StayID + `",` + field + `}`
		request := phase7Request(
...
func TestSelfRegistrationAcceptsGeneralizedDataAndAnswersPending(t *testing.T) {
	...
	handler := phase7Handler(t, phase7StayStub{selfRegistration: &captured}, nil)
	recorder := httptest.NewRecorder()
...
func TestActivationCompletionReturnsNoContentAndNoSession(t *testing.T) {
	...
	handler := phase7Handler(t, phase7StayStub{}, phase7ActivationStub{command: &captured})
...
// With the phase off the surfaces do not exist at all, which is a 404 rather
...
func TestDisabledPhaseDoesNotRegisterTheOpenChannel(t *testing.T) {
	...
	handler, _, err := httpapi.New(httpapi.Dependencies{
		...
		Stays:              stay.NewService(phase7StayStub{}),
	...
	for _, target := range []string{
		...
		if recorder.Code != http.StatusNotFound {
			t.Fatalf("%s with the phase off = %d, want 404", target, recorder.Code)
		}
	}
}
...
//
// This is the flow the phase exists for: the accommodation that received the
...
func TestActivatedAccountCanApproveItsOwnQueue(t *testing.T) {
	...
	for _, route := range []string{"approve", "reject"} {
		...
		request := phase7Request(
			http.MethodPost, "/api/v1/stays/"+phase7StayID+"/"+route, "activated", body,
		)
		request.Header.Set("If-Match", `"3"`)
		request.Header.Set("Idempotency-Key", "approval-key-0000000001")
		recorder := httptest.NewRecorder()
	...
	if captured.StayID.String() != phase7StayID {
		t.Fatalf("approval command = %#v", captured)
	}
}
...
func approveRequest(token string) *http.Request {
	request := phase7Request(
		http.MethodPost, "/api/v1/stays/"+phase7StayID+"/approve", token, "{}",
	)
	request.Header.Set("If-Match", `"3"`)
	request.Header.Set("Idempotency-Key", "approval-key-0000000001")
	return request
...
func TestOpenChannelRateLimitAnswersRetryAfter(t *testing.T) {
	...
	handler := phase7Handler(t, stateStayStub{err: stay.ErrRateLimited}, nil)
...
func TestApprovalMapsPreconditionAndIdempotencyConflicts(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			handler := phase7Handler(t, stateStayStub{err: test.err}, nil)
			recorder := httptest.NewRecorder()
...
func TestApprovalReplayIsAnnouncedAndReproducesTheAnswer(t *testing.T) {
	...
	phase7Handler(t, stateStayStub{}, nil).ServeHTTP(fresh, approveRequest("approver"))
	replay := httptest.NewRecorder()
	phase7Handler(t, stateStayStub{replayed: true}, nil).
...
func TestSelfRegistrationReplayReproducesTheAnswer(t *testing.T) {
	...
	fresh := httptest.NewRecorder()
	phase7Handler(t, stateStayStub{}, nil).
