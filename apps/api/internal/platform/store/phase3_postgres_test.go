...
func TestPhase3PostgreSQLSurveySecurityAndAtomicity(t *testing.T) {
	...
	fixture := seedPhase3Fixture(t, ctx, admin)
	phase3 := integrationPhase3Config()
	subject, err := store.NewPhase3(
		runtime,
		5*time.Second,
		integrationPhase2Config(t),
		phase3,
	)
	if err != nil {
		t.Fatalf("phase 3 store: %v", err)
	}
	repository := store.NewQuestionnaireRepository(subject)
	service := questionnaire.NewService(repository)
	codec := integrationCapabilityCodec(t, phase3)

	first := assertSurveyReplayCipherAndSinks(
		t, ctx, admin, service, codec, fixture,
	)
	assertSurveyPoolSaturationDoesNotStarve(
		t, ctx, admin, phase3, first,
	)
...
func assertSurveySingleConnectionPoolFailsClosed(
	...
	subject, err := store.NewPhase3(
		runtime, time.Second, integrationPhase2Config(t), phase3,
	)
	if err != nil {
		t.Fatalf("single-connection phase 3 store: %v", err)
	}
...
func assertSurveyPoolSaturationDoesNotStarve(
	...
	subject, err := store.NewPhase3(
		runtime, 2*time.Second, integrationPhase2Config(t), phase3,
	)
	if err != nil {
		t.Fatalf("limited phase 3 store: %v", err)
	}
	...
	for index := range commands {
		...
		commands[index].IdempotencyKey = fmt.Sprintf(
			"phase3-pool-saturation-key-%04d", index,
		)
		commands[index].RequestID = fmt.Sprintf(
			"phase3-pool-saturation-request-%04d", index,
		)
	...
	replay.RequestID = "phase3-pool-saturation-replay"
...
func seedPhase3Tenant(
	...
	if err != nil {
		t.Fatalf("seed phase 3 organization: %v", err)
	}
	_, err = admin.Exec(ctx,
	...
	if err != nil {
		t.Fatalf("seed phase 3 accommodation: %v", err)
	}
	_, err = admin.Exec(ctx,
		`INSERT INTO core.memberships
		   (id, accommodation_id, oidc_issuer, oidc_subject, role)
		 VALUES ($1, $2, 'https://issuer.invalid', 'phase3-manager', 'manager')`,
		fixture.membershipID,
		fixture.accommodationID,
	)
	if err != nil {
		t.Fatalf("seed phase 3 membership: %v", err)
	}
}

func seedPhase3Stays(
	t *testing.T,
	ctx context.Context,
	admin *pgxpool.Pool,
	fixture phase3Fixture,
	...
	for _, stayID := range fixture.stays {
		...
		if err != nil {
			t.Fatalf("seed phase 3 stay: %v", err)
		}
	}
	_, err := admin.Exec(ctx,
	...
	if err != nil {
		t.Fatalf("seed phase 3 visitor: %v", err)
	}
}
...
func seedPublishedQuestionnaire(
	...
	_, err := admin.Exec(ctx,
		`INSERT INTO survey.questionnaires (id, stable_key, name)
		 VALUES ($1, 'phase3_repository_profile', 'Pesquisa F3 repository')`,
		questionnaireID,
	)
	if err != nil {
		t.Fatalf("seed phase 3 questionnaire: %v", err)
	}
	_, err = admin.Exec(ctx,
	...
	if err != nil {
		t.Fatalf("seed phase 3 questionnaire version: %v", err)
	}
	seedPhase3Questions(t, ctx, admin, fixture)
	transitionSeededQuestionnaire(t, ctx, admin, fixture.versionID)
}

func seedPhase3Questions(
	t *testing.T,
	ctx context.Context,
	admin *pgxpool.Pool,
	fixture phase3Fixture,
	...
	if err != nil {
		t.Fatalf("seed phase 3 questions: %v", err)
	}
	_, err = admin.Exec(ctx,
	...
	if err != nil {
		t.Fatalf("seed phase 3 consent requirement: %v", err)
	}
}
...
func seedOtherQuestionnaireVersion(
	...
	_, err := admin.Exec(ctx,
		`INSERT INTO survey.questionnaires (id, stable_key, name)
		 VALUES ($1, 'phase3_cross_version', 'Questionário de controle')`,
		questionnaireID,
	)
	if err != nil {
...
func integrationPhase3Config() config.Phase3Config {
	return config.Phase3Config{
...
func integrationCapabilityCodec(
	...
	codec, err := questionnaire.NewCapabilityCodec(questionnaire.Keyring{
		CurrentVersion: phase3.SurveyKeys.CurrentVersion,
		Keys:           phase3.SurveyKeys.Keys,
	})
	if err != nil {
...
func assertSurveyReplayCipherAndSinks(
	...
	t.Helper()
	const canary = "free-text-canary-phase3-never-plaintext"
	...
	command := submittedSurveyCommand(
		...
		"phase3-submit-replay-key-0001",
		"phase3-submit-request-0001",
		canary,
	)
	...
	replay := command
	replay.RequestID = "phase3-submit-request-0002"
...
func assertSurveyReplayBypassesConsumedRateBudget(
	...
	command := submittedSurveyCommand(
		t, fixture, capability,
		"phase3-rate-replay-key-0001",
		"phase3-rate-replay-request-0001",
		"rate-replay-canary",
	)
...
func consumeSurveyRateBudget(
	...
	for attempt := 2; attempt <= 10; attempt++ {
		probe := command
		probe.IdempotencyKey = fmt.Sprintf("phase3-rate-new-key-%04d", attempt)
		probe.RequestID = fmt.Sprintf("phase3-rate-new-request-%04d", attempt)
	...
	limited := command
	limited.IdempotencyKey = "phase3-rate-over-limit-key-0011"
	limited.RequestID = "phase3-rate-over-limit-request-0011"
...
func assertSurveyReplayAfterLimit(
	...
	t.Helper()
	replay := command
	replay.RequestID = "phase3-rate-replay-after-limit"
...
func assertConsumedCapabilityRejectsNewKey(
	...
	command.IdempotencyKey = "phase3-consumed-new-key-0001"
	command.RequestID = "phase3-consumed-request-0001"
...
func assertSurveyCapabilityVersionBinding(
	...
	command := submittedSurveyCommand(
		t, fixture, capability,
		"phase3-wrong-version-key-0001",
		"phase3-wrong-version-request-0001",
...
func assertConcurrentSurveyConsumption(
	...
	commands := []questionnaire.SubmissionCommand{
		submittedSurveyCommand(
			t, fixture, capability,
			"phase3-concurrent-key-0001",
			"phase3-concurrent-request-0001",
			"concurrent-canary-a",
		),
		submittedSurveyCommand(
			t, fixture, capability,
			"phase3-concurrent-key-0002",
			"phase3-concurrent-request-0002",
			"concurrent-canary-b",
		),
	}
...
func assertSurveyRollbackAfterAnswers(
	...
	command := submittedSurveyCommand(
		t, fixture, capability,
		"phase3-rollback-key-0001",
		"phase3-rollback-request",
		"rollback-canary",
	)
...
func installAuditFailureTrigger(
	...
	t.Helper()
	_, err := admin.Exec(ctx,
		`CREATE OR REPLACE FUNCTION platform.phase3_test_fail_audit()
		 RETURNS trigger LANGUAGE plpgsql AS $$
		 BEGIN
		   IF NEW.request_id = 'phase3-rollback-request' THEN
		     RAISE EXCEPTION 'phase3 injected audit failure';
		   END IF;
		   RETURN NEW;
		 END;
		 $$;
		 CREATE TRIGGER phase3_test_fail_audit
		 BEFORE INSERT ON platform.audit_events
		 FOR EACH ROW EXECUTE FUNCTION platform.phase3_test_fail_audit()`,
	)
	if err != nil {
...
func removeAuditFailureTrigger(
	...
	t.Helper()
	_, err := admin.Exec(ctx,
		`DROP TRIGGER phase3_test_fail_audit ON platform.audit_events;
		 DROP FUNCTION platform.phase3_test_fail_audit()`,
	)
	if err != nil {
...
func assertDeclineDoesNotBlockCheckIn(
	...
	command := questionnaire.SubmissionCommand{
		...
		IdempotencyKey: "phase3-decline-key-0001",
		RequestID:      "phase3-decline-request-0001",
	}
	result, replayed, err := service.Submit(ctx, command)
	...
	transition, replayed, err := stayService.Transition(ctx, stay.TransitionCommand{
		Actor: principal("phase3-manager"), StayID: capability.stayID,
		ExpectedVersion: 1, Kind: stay.TransitionCheckIn,
		OccurredAt:     time.Now().UTC(),
		IdempotencyKey: "phase3-check-in-key-0001",
		RequestID:      "phase3-check-in-request-0001",
	})
...
func assertRetiredVersionAllowsReplay(
	...
	replay := accepted.command
	replay.RequestID = "phase3-retired-replay-request"
	...
	other.Capability = issueSurveyCapability(
		t, ctx, admin, integrationCapabilityCodec(t, integrationPhase3Config()),
		fixture.stays[6], fixture.versionID,
	).Token
	other.IdempotencyKey = "phase3-retired-new-key-0001"
	other.RequestID = "phase3-retired-new-request-0001"
