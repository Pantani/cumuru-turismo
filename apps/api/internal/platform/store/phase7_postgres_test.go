...
func TestPhase7ApprovingARejectedStayIsRefused(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 90*time.Second)
	defer cancel()
	admin, runtime, fixture := openPhase7Integration(t, ctx)
	repository := newPhase7Repository(t, runtime)

	accepted := submitPhase7SelfRegistration(t, ctx, repository, fixture)
	rejected, _, err := repository.Reject(ctx, stay.RejectionCommand{
		...
		IdempotencyKey: "phase7-reject-first-" + accepted.stayID.String(),
	...
	if _, _, err := repository.Approve(ctx, stay.ApprovalCommand{
		...
		IdempotencyKey:  "phase7-approve-after-reject-" + accepted.stayID.String(),
...
func TestPhase7ActivationCapabilityIsSingleUseUnderConcurrency(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 120*time.Second)
	defer cancel()
	admin, runtime, fixture := openPhase7Integration(t, ctx)
	repository := newPhase7ActivationRepository(t, runtime)
