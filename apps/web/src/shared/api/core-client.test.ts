...
import { createCoreClient, coreOperationNames } from "./core-client";

const token = "access-token-used-only-in-memory";
const requestId = "request-core-client-test";
...
type ContractClient = ReturnType<typeof createCoreClient>;
...
describe("cliente tipado do núcleo", () => {
  ...
  it("aceita cadastro concluído quando não existe pesquisa publicada", async () => {
    ...
    const client = createCoreClient({ fetcher, getAccessToken: () => null });
