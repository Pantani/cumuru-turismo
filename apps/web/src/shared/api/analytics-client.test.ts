...
import { createAnalyticsClient, type AnalyticsClient, analyticsOperationNames } from "./analytics-client";

type Schemas = components["schemas"];

const requestId = "request-analytics-client-test";
const publicHeaders = {
...
interface InvalidPayloadCase {
  ...
  invoke: (client: AnalyticsClient) => Promise<unknown>;
  name: string;
  payload: unknown;
}
...
describe("cliente tipado de analytics", () => {
  it("declara exatamente as seis operações do contrato congelado", () => {
    ...
    >();
    expect(analyticsOperationNames).toEqual([
      "getPublicSummary",
      "getPublicPresence",
  ...
  it.each([
    {
      name: "presença",
      payload: presence,
      invoke: (client: AnalyticsClient) =>
        client.getPresence("recent_30_days"),
    },
    {
      name: "preferências",
      payload: preferences,
      invoke: (client: AnalyticsClient) => client.getPreferences(),
    },
    {
      name: "metodologia",
      payload: methodology,
      invoke: (client: AnalyticsClient) => client.getMethodology(),
    },
