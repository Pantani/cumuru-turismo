...
import { ACTIVATION_TOKEN_HEADER, INVITE_TOKEN_HEADER, createSelfServiceClient, selfServiceOperationNames } from "./self-service-client";

const accessToken = "access-token-used-only-in-memory";
const requestId = "request-self-service-client-test";
...
type ContractClient = ReturnType<typeof createSelfServiceClient>;
...
describe("cliente tipado do autoatendimento", () => {
  it("declara exatamente as 10 operações do contrato da fase", () => {
    expectTypeOf(selfServiceOperationNames).toMatchTypeOf<
      ReadonlyArray<keyof operations>
    >();
    expect(selfServiceOperationNames).toHaveLength(10);
    expect(new Set(selfServiceOperationNames).size).toBe(10);
  });
  ...
  it.each(contractCases())(
    ...
    async (_operation, method, path, status, headers, invoke) => {
      ...
      const client = createSelfServiceClient({
  ...
  it("recusa capability fora do formato antes de qualquer requisição", async () => {
    ...
    const client = createSelfServiceClient({ fetcher, getAccessToken: () => null });
